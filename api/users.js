...
try { const userResponse = await supabase.auth.getUser(token); if (userResponse.error) { console.error('Auth error:', userResponse.error); return res.status(401).json({ error: 'Invalid or expired token' }); }
...
} catch (err) { console.error('Serverless function error:', err); return res.status(500).json({ error: err.message || 'Internal server error' }); } };
